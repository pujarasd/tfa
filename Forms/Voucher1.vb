Public Class Voucher1
    ...
    Sub Nav_Data()
        ...
        If mode = enEntry.Edit Or mode = enEntry.Add Then
            ...
            btnSave.Text = "&Save"
            If mode = enEntry.Add Then
                If cmbAccount.Items.Count = 3 Then
                    cmbAccount.SelectedIndex = 2
                Else
                    If bs.Count > 1 Then
                        bs.MovePrevious()
                        Dim aid As Integer = bs.Current("AccId")
                        bs.MoveNext()
                        cmbAccount.SelectedValue = aid
                    End If
                End If
            End If
    ...
    Sub Fill_Combo()
        ...
        Else
          ...
          " select mledger.id,mledger.ledger+' ~ '+cte.groupname as ledger from cte inner join mledger on mledger.groupid = cte.id order by ledger "
            'Dim dvgroup As DataView = dbTable("select * from mledgergroup").DefaultView
            'Dim dvgroup1 As DataView = dbTable("select * from mledgergroup").DefaultView
            'Dim dt As New DataTable
            'dt.Columns.Add("Id", Type.GetType("System.Int32"))
            'dt.Columns.Add("Ledger", Type.GetType("System.String"))
            'Dim dv As DataView = dbTable("Select * from mledger").DefaultView
            'For i = 0 To dvgroup.Count - 1
            '    If dvgroup(i)("id") = 19 Then
            ...
            For i = 1 To 5
                ...
                Next
            Next
            fill_combocommon(cmbAccount, "Select a.id,(a.ledger +'~' +b.groupname) as ledger  from mledger a inner join mledgergroup b on b.id = a.groupid where a.groupid in (" & strgroup.Substring(0, strgroup.Length - 1) & ")", "ledger", "id")
    ...
    Private Sub chkNarration_CheckStateChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkNarration.CheckStateChanged
        If Not isLoading Then
            If chkNarration.CheckState = CheckState.Checked Then
                ...
                pnlBank.Enabled = IIf(vType.Contains("Bank"), True, False)
                txtBank.Enabled = IIf(vType.Contains("Bank Receipt"), True, False)
                txtBranch.Enabled = IIf(vType.Contains("Bank Receipt"), True, False)
            ...
            Else
                ...
                pnlBank.Enabled = IIf(vType.Contains("Bank"), True, False)
                pnlNarration.Enabled = False
                'dgv.Height = dgv.Height + pnlNarration.Height + pnlBank.Height
                'dgv.Top = pnlBank.Top
                dgv.Columns("Narration").Visible = True
                dgv.Columns("Bank").Visible = IIf(vType.Contains("Bank Receipt"), True, False)
                dgv.Columns("Branch").Visible = IIf(vType.Contains("Bank Receipt"), True, False)
        ...
        ElseIf e.ColumnIndex = 2 Or e.ColumnIndex = 3 Then
            If vType.Contains("Cash") Or vType.Contains("Bank") Or vType.Contains("Journal") Then
                ...
                If nz(dbValue("Select BBB from mledger where id = " & accid), False) Then
                    ...
                    dtref = dbTable("Select * from trefdata where accid = " & bsd.Current("AccId") & " and dataid = " & bsd.Current("VoucherId") & " and lineid =" & bsd.Current("LineId"))
                    ofrm.dtref = dtref
                    ofrm.accId = accid
                    ofrm.adj_amt = dgv(e.ColumnIndex, e.RowIndex).Value
                    ofrm.dataid = 0
                    ofrm.lineid = e.RowIndex
                    ofrm.type = dgv(0, e.RowIndex).Value
                    ofrm.refdate = dtpDate.Value
                    ofrm.RefNo = vPrefix & numVoucherNo.value & vSuffix
                    ofrm.ShowDialog()
    ...
    Function save_entry() As Boolean
        ...
        ElseIf mode = enEntry.Edit Then
            ...
            ,CoId =  <%= gCoId %>        ,Type='<%= vDrCr %>', period = '<%= gPeriod %>',modifyby='<%= gUserName %>' 
           where id = <%= txtId.Text %>
                  </string>
    ...
    Private Sub dgv_RowValidated(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv.RowValidated
        ...
        If dtref IsNot Nothing Then
            For Each refrow In dtref.Rows
                If refrow("RefType") = "New" And refrow("DataId") = 0 And refrow("LineId") = 0 Then
                    refrow("DataId") = txtId.Text ' dgv.Rows(e.RowIndex).Cells("Id")

                    refrow("LineId") = e.RowIndex
                End If
            Next
        End If
    ...
    Private Sub tsbNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tsbNew.Click
        bs.MoveLast()
        Dim vno As Integer = nz(bs.Current("voucherno"), 0) + 1
    ...
    Private Sub tbNav_ItemClicked(ByVal sender As Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tbNav.ItemClicked
        ...
        Select Case e.ClickedItem.Name
            ...
            Case "tsbNext"
                navmode = enNavMode.PrevRec
            Case "tsbGo"
                If tstChallan.Text.Trim <> "" Then
                    bs.MoveFirst()
                    If bs.Current("VoucherNo") <> tstChallan.Text Then
    ...
    '    ''dr("voucherno") = nz(bs.Current("voucherno"), 0) + 1
    '    ''e.NewObject = dr
    '    'dt = Nothing
    '    'dr = Nothing
    '    Dim drv As DataRowView = DirectCast(bs.List, DataView).AddNew()
    '    bs.MoveLast()
    '    drv("coid") = gCoId
    '    drv("Voucher") = vName
    '    drv("VoucherNo") = nz(bs.Current("voucherno"), 0) + 1
    '    drv("VoucherPrefix") = vPrefix
    '    drv("Date") = Now.Date
    '    drv("Type") = vDrCr
